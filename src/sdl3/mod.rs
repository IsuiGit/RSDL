...
impl SDL3{
    pub fn new() -> Self {
        unsafe {
            let lib = Library::new(Path::new("src/sdl3/bin/SDL3.dll")).expect("Failed to load SDL3.dll");
            SDL3 {lib}
        }
    }
    ...
    pub fn sdl3_create_renderer(&mut self, window: *mut c_void, name: &str) -> *mut c_void {
        unsafe{
            ...
            let _sdl3_create_renderer: Symbol<SDL_CreateRenderer> = self.lib.get(b"SDL_CreateRenderer").expect("Failed to get symbol SDL_CreateRenderer");
    ...
    pub fn sdl3_destroy_renderer(&mut self, renderer: *mut c_void) {
        unsafe {
            let _sdl3_destroy_renderer: Symbol<SDL_DestroyRenderer> = self.lib.get(b"SDL_DestroyRenderer").expect("Failed to get symbol SDL_DestroyRenderer");
